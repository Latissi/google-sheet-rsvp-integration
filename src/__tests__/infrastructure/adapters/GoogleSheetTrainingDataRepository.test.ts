...
describe('GoogleSheetTrainingDataRepository', () => {
  ...
  it('parses training definitions and sessions from member-row sheets', () => {
  ...
  it('reads attendance and metadata from member rows', () => {
    const { repository } = createRepository();
  ...
  it('matches member rows even when public names contain symbols', () => {
    const symbolUsers = [createUser(createCompositeMemberId('Carla', 'Sommer'), 'Carla Sommer')];
    const gateway = new MockSheetGateway({
      'RSVP Übersicht': [
  ...
  it('marks a member-row session as cancelled via the header note', () => {
    const { gateway, repository } = createRepository();
    ...
    expect(gateway.getCellNote('RSVP Übersicht', 1, 3)).toBe('{"cancelledAt":"2026-03-10T12:00:00.000Z","cancelledByMemberId":"charlie::coach","reason":"Unwetter"}');
    expect(repository.getTrainingSessionById('club-rsvp__wed-mixed__2026-03-11__18:00')).toEqual({
      sessionId: 'club-rsvp__wed-mixed__2026-03-11__18:00',
      trainingId: 'wed-mixed',
      sessionDate: '2026-03-11',
      startTime: '18:00',
